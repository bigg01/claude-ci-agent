...
=== "GitHub Actions"
    ...
    **Use it** — pass it into the action via the `secrets` context (it's masked in
    ...
    ```yaml
    - uses: bigg01/claude-ci-agent@v0.1.0-alpha.10
      with:
...
=== "GitLab CI"
    ...
    **Use it** — the [component](#gitlab-ci-using-the-claude-agent-component) reads
    ...
    ```yaml
    include:
      - component: $CI_SERVER_FQDN/<group>/claude-ci-agent/claude-agent@v0.1.0-alpha.10
        inputs:
          prompt: "Fix the failing tests."
    ```
...
| `image` | `ghcr.io/bigg01/claude-ci-agent/claude-agent:0.1.0-alpha.10` | Published sandbox image providing the Claude Code CLI + baked CI helpers. |
...
| `token_variable` | `GITLAB_TOKEN` | **Name** of the variable holding a GitLab token used to push the branch and open/comment on MRs. Needs `write_repository` **and** `api` scopes and at least the **Developer** role (a read-only token gets a 403 on push). `CI_JOB_TOKEN` is not sufficient. |
...
!!! note "Replace the component path and pin a version"
    ...
    `@v0.1.0-alpha.10` to a released tag (or a commit SHA) for reproducible pipelines.
...
variables:
  AGENT_IMAGE: ghcr.io/bigg01/claude-ci-agent/claude-agent:0.1.0-alpha.10
