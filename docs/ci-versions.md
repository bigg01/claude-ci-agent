...
=== "GitHub Actions"
    ...
    **Use it** — pass it into the action via the `secrets` context (it's masked in
    ...
    ```yaml
    - uses: bigg01/claude-ci-agent@v0.1.0-alpha.13
      with:
...
| `image` | `ghcr.io/bigg01/claude-ci-agent/claude-agent:0.1.0-alpha.13` | Published sandbox image providing the Claude Code CLI + baked CI helpers. |
