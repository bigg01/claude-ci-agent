...
runs) and the **OTel Collector sidecar**. Inside the sandbox, the agent drives
`podman build`/`run`, which spawns **nested child containers** (rootless-in-rootless)
to build and test the app under review. Claude never runs *in* those child
containers — it orchestrates them from the sandbox.
...
| **Podman sandbox** *(Container 1)* | Rootless, unprivileged execution environment built from the `claude-agent` image. **The Claude CLI runs here**, alongside Podman. No Docker. |
